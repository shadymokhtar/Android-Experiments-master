...
public class MyAccount extends Fragment implements OnClickListener,URLThread_CallBack,LocationListener
{
	...
	public final String AppAccessToken="ARN0JDFEXTLW4BKPGVCVLFYSTJTS3BRFTW3S2CWUL0FICCCF";
...
return v;
	...
	public void onLocationChanged(Location location) 
	{
		...
		Date d=new Date();
		urlThread_checkLocation=new URLThread("https://api.foursquare.com/v2/venues/search?ll="+location.getLatitude()+","+location.getLongitude()+"&oauth_token="+AppAccessToken+"&v=20130218", this, "");
	...
	{
		//TODO just testing
// 	   ((TextView) v.findViewById(R.id.tv_currentLocation)).setText("URLCallBack:"+response.substring(0, Math.min(50,response.length())));
		try
		{
			if(urlThread_checkLocation!=null)
			{
			//	response = streamToString(connection.getInputStream());  ///////edittttt
		//		JSONObject jsonObj  = (JSONObject) new JSONTokener(response).nextValue();   //////editttt 
//				System.out.println(">>>><<"+new JSONObject(response).toString());
				 final JSONArray venues= new JSONObject(response).getJSONObject("response").getJSONArray("venues");
				...
				getActivity().runOnUiThread(new Runnable()
					{
						...
						public void run() 
						{
							try{
								...
								String tmp[]=new String[venues.length()];
								for(int i=0;i<venues.length();i++)
									tmp[i]=venues.getJSONObject(i).getString("name");
								final String ar[]=tmp;
